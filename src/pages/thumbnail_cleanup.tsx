...
data.get("/", async (c) => {
  ...
  try {
    thumbnailsBeforeLastYearAndOnlyMaybeRepliedResult = await db
      ...
      .where(
        and(
          ilike(media.thumbnailUrl, `${storageUrlBase}%`),
  ...
  try {
    thumbnailsCountResult = await db
      ...
      .from(media)
      .where(ilike(media.thumbnailUrl, `${storageUrlBase}%`));
  } catch {
...
data.post("/clean_preview", async (c) => {
  ...
  if (typeof beforeParameter === "string") {
    ...
    if (owner != null && storageUrlBase !== undefined) {
      logger.info(`Starting cleanup preview - before: ${before.toISOString()}`);
      try {
        const mediaToDelete: { id: Uuid; key: string | null; created: Date }[] =
          (await readFilesToDelete(before, storageUrlBase)).map((row) => ({
            id: row.id,
            key: row.thumbnailUrl.startsWith(storageUrlBase as string)
              ? row.thumbnailUrl.replace(storageUrlBase as string, "")
              : null,
            created: row.created,
          }));
